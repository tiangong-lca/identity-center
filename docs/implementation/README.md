...
| 数据库兼容 | **PostgreSQL 与 KingbaseES 双库均为一等公民**：迁移与数据访问必须在两种数据库上实测通过，不是仅遵守书面约定 |
...
- 数据库兼容：全部迁移与核心数据访问在 PostgreSQL 与 KingbaseES（PostgreSQL 兼容模式）双库实测通过。
...
|---|---|---|---|
| L0 基础设施 | 4-6 天 | 未开始 | - |
| L1 数据层（双库） | 7-10 天 | 未开始 | - |
| L2 基础能力层 | 6-10 天 | 未开始 | - |
| L3 服务与任务层 | 11-16 天 | 未开始 | - |
| L4 接口层 | 8-12 天 | 未开始 | - |
| L5 页面层（i18n + 主题） | 15-21 天 | 未开始 | - |
| L6 联调与 Supabase 接入 | 5-8 天 | 未开始 | - |
| L7 安全加固与上线 | 6-9 天 | 未开始 | - |

总概量约 62-92 个工作日（单人 + AI 辅助，约 3.5-4.5 个月）。状态随实施更新，每层完成时在本表记录日期与对应 commit。
