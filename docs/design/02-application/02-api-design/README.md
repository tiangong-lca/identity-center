...
`POST /api/admin/applications`、`PATCH /api/admin/applications/{id}`、`POST /api/admin/applications/{id}/roles`、`PATCH /api/admin/applications/{id}/roles/{roleId}` 这 4 个定义写接口现在统一返回 `409 CATALOG_MANAGED`，不再直接写库。应用/角色定义唯一的编辑路径是 §6.2 的业务应用目录 API（管理后台 `/admin/catalog` 控制台）；应用详情页对应的“基础信息”“角色目录”标签页改为只读展示，并提供跳转到目录编辑器的入口，应用准入、角色分配等 tab 不受影响。

### 6.2 业务应用目录 API

```text
GET  /api/admin/catalog                    权限 catalog:read，返回当前目录 YAML 快照与版本号
GET  /api/admin/catalog/versions           权限 catalog:read，返回版本历史列表
GET  /api/admin/catalog/versions/{version} 权限 catalog:read，返回指定版本的完整记录
POST /api/admin/catalog/apply              权限 catalog:apply，提交 YAML + expectedVersion 做乐观并发校验后落库
POST /api/admin/catalog/rollback           权限 catalog:apply，回滚到指定历史版本（同样走 expectedVersion 校验）
```

管理后台 `/admin/catalog` 页面以 Monaco 编辑器承载：载入当前 YAML → 编辑 → Apply（乐观并发冲突返回 `409 CONFLICT`，校验失败返回 `400 VALIDATION_ERROR` 并带 `issues` 明细）→ 展示本次变更 diff；`apply`/`rollback` 未产生实际变化时按 no-op 处理，不追加新版本、不显示误导性的成功提示。页面同时提供版本历史查看与回滚入口。

### 6.3 待停用确认 API

```text
GET  /api/admin/catalog/pending-deactivate 权限 catalog:read，返回当前处于 pending_deactivate 的应用/角色清单
POST /api/admin/catalog/deactivate         权限 catalog:apply，对指定应用/角色执行确认停用（置为 deactivated 终态）
```

`deactivate` 只接受已处于 `pending_deactivate` 的目标，写入前经明文 secret 扫描（见 §4.10 `lib/catalog/secret-scan.ts`），命中则拒绝写入并在响应中列出违规字段；确认停用是终态操作，不提供反向恢复接口。
...
| CONFLICT | 409 | 状态冲突 |
| CATALOG_MANAGED | 409 | 应用/角色定义改由目录管理，定义写接口不再直接生效（见 §6.1） |
